...
CREATE INDEX idx_members_active ON members (name, type) WHERE is_active = TRUE;

-- Index only available stock
CREATE INDEX idx_purchase_available ON purchase (item_name, purchase_date) WHERE remaining_quantity > 0;
